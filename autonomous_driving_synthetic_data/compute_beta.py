...
class beta_cem():
    ...
    def __init__(self,num_reduced,num_mother,ker_wt,P_jax):
        ...
        self.compute_beta_reduced_vmap = jit(vmap(self.compute_beta_reduced,in_axes=(0,0)))
    ...
    def compute_cem(self, cx_mother,cy_mother,x_mother,y_mother):
        ...
        def lax_cem(carry,idx):
            ...
            cx_red = vmap(lambda i,j:  cx_mother_stack[i][j])(jnp.arange(self.num_samples_cem),idx_beta_top) # num_samples_cem x red_set x 11
            cy_red = vmap(lambda i,j:  cy_mother_stack[i][j])(jnp.arange(self.num_samples_cem),idx_beta_top) # num_samples_cem x red_set x 11
            
            x_red = vmap(lambda i,j:  x_mother_stack[i][j])(jnp.arange(self.num_samples_cem),idx_beta_top) # num_samples_cem x red_set x num
            y_red = vmap(lambda i,j:  y_mother_stack[i][j])(jnp.arange(self.num_samples_cem),idx_beta_top) # num_samples_cem x red_set x num
            ...
            ker_red,ker_mixed,_ = self.prob.compute_kernel_vmap(A,B,sigma)

            beta_top,cost_batch = self.compute_beta_reduced_vmap(ker_red,ker_mixed) # num_samples_cem x red_set
