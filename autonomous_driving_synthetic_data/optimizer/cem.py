...
class CEM():
    def __init__(self,num_reduced,num_mother,num_obs,setting):
        ...
        ### can modify this block as per your requirements
        if setting=="inlane_cut_in_low" or setting=="inlane_cut_in_high":
            self.y_lb,self.y_ub = -2.25,-1.25
        else:
            self.y_lb,self.y_ub = -2.25,2.25 # for lane_change(dynamic obs) or static obs 
        ...
        self.weight_mmd_lane,self.weight_mmd_obs = 10**(3),10**(3)
        self.weight_cvar_lane,self.weight_cvar_obs = 10**(6),10**(6)
        self.weight_saa_lane,self.weight_saa_obs = 10**(6),10**(6)

        self.sigma_ker = 10**(-2)
        self.ker_wt = 1000.

        self.prob = kernel_matrix(num_reduced,self.ker_wt,self.P_jax)
        
        self.prob2 = beta_cem(num_reduced,num_mother,self.ker_wt,self.P_jax)
        ...
        self.projection = Projection(self.num_obs,self.num_circles,self.v_max,self.v_min,self.a_max,
                 ...
                 self.A_vel,self.A_acc,self.A_projection,self.num_reduced)
        ...
        self.cem_helper = Helper(self.v_min,self.v_max, self.P,self.Pdot,self.Pddot,self.rho_v,self.rho_offset,
                 ...
                 self.steer_max,self.nvar,self.t,self.num_mother,self.prob,self.prob2)
        ...
        self.costs = Costs(self.prob,self.num_reduced,self.num_obs,self.num_prime,
                           ...
                           self.y_des_1,self.y_des_2,self.ellite_num_projection)

    @partial(jit, static_argnums=(0, ))	
    def compute_cem_mmd_opt(self,idx_mpc,init_state,
                    mean_param_init,cov_param_init,
                    x_obs_traj,y_obs_traj,v_des,beta,sigma):
        ...
        def lax_cem(carry,idx):
            ...
            c_x, c_y, x, y, xdot, ydot, xddot, yddot, res_norm_batch,lamda_x, lamda_y,s_lane\
                  = self.projection.compute_projection(x_obs_traj.reshape(self.num_obs*self.num_circles*self.num_reduced,self.num),
                                y_obs_traj.reshape(self.num_obs*self.num_circles*self.num_reduced,self.num),
                                b_eq_x, b_eq_y,lamda_x, lamda_y, c_x_bar, c_y_bar,self.a_obs,self.b_obs,s_lane)
            ...
            mmd_obs = self.costs.compute_mmd_obs(beta,x_roll_ellite_projection,y_roll_ellite_projection,
                                              x_obs_traj[:,:,0:self.num_prime],y_obs_traj[:,:,0:self.num_prime],sigma)
            ...
            cost_batch = self.cem_helper.compute_cost(self.weight_mmd_obs*mmd_obs_ellite,x_ellite_mmd,y_ellite_mmd, res_ellite_mmd, \
            ...
            return (res,res_2,lamda_x, lamda_y, neural_output_batch,mean_param,cov_param,s_lane),\
                (c_x_ellite_mmd[idx_min],c_y_ellite_mmd[idx_min],mmd_obs_ellite[idx_min])
        ...
        return cx_best,cy_best,mmd_obs
    ...
    def compute_cem_cvar(self,idx_mpc,init_state,
        ...
        def lax_cem(carry,idx):
            ...
            mmd_obs = self.costs.compute_cvar_obs(x_roll_ellite_projection,y_roll_ellite_projection,
    ...
    def compute_cem_saa(self,idx_mpc,init_state,
        ...
        def lax_cem(carry,idx):
            ...
            mmd_obs = self.costs.compute_saa_obs(x_roll_ellite_projection,y_roll_ellite_projection,
                                              x_obs_traj[:,:,0:self.num_prime],y_obs_traj[:,:,0:self.num_prime])
