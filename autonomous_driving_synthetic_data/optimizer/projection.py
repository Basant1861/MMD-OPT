...
class Projection():
    def __init__(self,num_obs,num_circles,v_max,v_min,a_max,
                 ...
                 A_vel,A_acc,A_projection,num_reduced):
    ...
    def initial_alpha_d_obs(self, a_obs,b_obs,x_guess, y_guess, xdot_guess, ydot_guess, xddot_guess, yddot_guess, 
        ...
        wc_alpha = wc_alpha.reshape(self.num_batch, self.num*((self.num_obs)*self.num_reduced*self.num_circles))
        ws_alpha = ws_alpha.reshape(self.num_batch, self.num*((self.num_obs)*self.num_reduced*self.num_circles))
        ...
        d_temp = c2_d/c1_d
        d_obs = jnp.maximum(jnp.ones((self.num_batch,  self.num*self.num_reduced*self.num_obs*self.num_circles   )), d_temp   )
    ...
    def compute_x(self, a_obs,b_obs,lamda_x, lamda_y, b_eq_x, b_eq_y, alpha_a, d_a, alpha_v, d_v, x_obs, y_obs,
        ...
        b_obs_x = x_obs.reshape(self.num_reduced*self.num*self.num_obs*self.num_circles)+temp_x_obs
            
        temp_y_obs = d_obs*jnp.sin(alpha_obs)*b_obs
        b_obs_y = y_obs.reshape(self.num_reduced*self.num*self.num_obs*self.num_circles)+temp_y_obs
    ...
    def comp_d_obs_prev(self,d_obs_prev):
        d_obs_batch = jnp.reshape(d_obs_prev,(self.num_batch,self.num_reduced*self.num_circles*self.num_obs,self.num))
        d_obs_batch_modified = jnp.dstack((jnp.ones((self.num_batch,self.num_reduced*self.num_circles*self.num_obs,1)),
                                            d_obs_batch[:,:,0:self.num-1]))
    ...
    def compute_alph_d(self,x_obs, y_obs, x, y, xdot, ydot, xddot, yddot, lamda_x, lamda_y,
        ...
        wc_alpha = wc_alpha.reshape(self.num_batch, self.num_reduced*self.num*((self.num_obs)*self.num_circles))
        ws_alpha = ws_alpha.reshape(self.num_batch, self.num_reduced*self.num*((self.num_obs)*self.num_circles))
        ...
        d_obs = jnp.maximum(jnp.ones((self.num_batch,self.num_reduced*self.num*((self.num_obs)*self.num_circles)  )) \
                            + (1-self.gamma_obs)*(d_obs_prev-1),d_temp)
