...
for sc in list_setting:
    ...
    for k in range(0,common_set.shape[0]):
        ...
        for i,cost in enumerate(list_costs):
            ...
            for tt in range(num_obs):
                ...
                count = jnp.max(intersection) + count
            ...
            for jj in range(0,num,3):
                ...
                for ii in range(num_obs):
                    ...
                    axs.plot(_x_obs_noise[ii].T,_y_obs_noise[ii].T,color = "m",lw = 1,ls = "--")
                    _obs_traj, = axs.plot(_x_obs_noise[ii][0],_y_obs_noise[ii][0],color = "m",
                                        lw = 1,ls = "--",label="Obstacle trajectory \n samples")
                    
                    for tt in range(0,_n):
                        patch_obs = pt.Rectangle((_x_obs_noise[ii][tt,jj]-2.5,_y_obs_noise[ii][tt,jj]-1.25),
                                                5,2.5,np.rad2deg(_psi_obs_noise[ii][tt,jj]), 
                                            fc = "red",ec = "black",linewidth = 0.5,
                                    label="Obstacle")
                
                        axs.add_patch(patch_obs)

                _traj, = axs.plot(x_best[0],y_best[0],"g*",markersize = 12, label="Ego start position")
                ego_traj, = axs.plot(x_best,y_best, "b", linewidth=3, label="Ego trajectory")

        ####### Ego patches
                patch_ego = pt.Rectangle((x_best[jj]-2.5,y_best[jj]-1.25), 5,2.5,np.rad2deg(psi_best[jj]), 
                                            fc = "orange",ec = "black",linewidth = 2,
                                    label="Ego")
                
                axs.add_patch(patch_ego)

            ###############################
                axs.text(0,17,'{}% collisions \n ({}/{})'.format(int(100*count/1000),int(count),_num_batch))
                axs.text(0,8,'Ego speed {} m/s'.format(round(v_best[jj],2)))
                ...
                #         axs.annotate('High probability obstacle trajectories \n ({}%)'.format(round((num_prob_1/_num_batch)*100),2), 
                #                         xy=(80,-1.2), xytext=(70,-23),
                #     arrowprops=dict(facecolor='black', shrink=0.05),fontsize=fs)
            
                #     else:
                #         axs.annotate('Low probability obstacle trajectories \n ({}%)'.format(round((num_prob_1/_num_batch)*100),2), 
                #                                 xy=(80,-1.2), xytext=(70,-23),
                #                                 arrowprops=dict(facecolor='black', shrink=0.05),fontsize=fs)
                        
                axs.legend(handles=[ego_traj,patch_obs,_traj,lb,patch_ego,_obs_traj],loc='upper right',ncol=3,fontsize="medium")

                camera.snap()

            animation = camera.animate()
            animation.save('videos/{}_{}_config_{}.mp4'.format(cost,sc,k))
