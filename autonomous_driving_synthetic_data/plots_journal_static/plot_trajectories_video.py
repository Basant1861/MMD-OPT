...
#########################################
        ...
        for i,cost in enumerate(list_cost):
            ...
            camera = Camera(fig)
            for j,ts in enumerate(range(0,num,3)):
                print(j)
        ...
        ### Adding uncertainty trajectories
                ...
                for ii in range(num_obs):
                    _x_obs_noise_shift = X_obs[ii,0:_n,0] - 2.5
                    _y_obs_noise_shift = Y_obs[ii,0:_n,0] - 1.25 

                    patches_noise = [pt.Rectangle(center, width,height) for center, width,height in \
                    zip(np.vstack((_x_obs_noise_shift,_y_obs_noise_shift)).T, 5*np.ones(_n),2.5*np.ones(_n))]        
                    
                    coll = matplotlib.collections.PatchCollection(patches_noise,fc='red',alpha = 0.01)
                    axs.add_collection(coll)
                
                _traj, = axs.plot(x_best[0],y_best[0],"g*",markersize = 7, label="Ego start position")

            ####### Ego patch           _
                if cost=="mmd_opt":
                            
                    ego_traj, = axs.plot(x_best[0:int(ts)],y_best[0:int(ts)]
                                            , "b", linewidth=3, label="Ego trajectory")    
                    patch_ego = pt.Rectangle((x_best[int(ts)]-2.5,y_best[int(ts)]-1.25), 
                                            5,2.5,np.rad2deg(psi_best[int(ts)]), fc = "orange",ec = "black",linewidth = 2,
                                                label="Ego")
                    axs.add_patch(patch_ego)
                    ...
                    ego_traj, = axs.plot(x_best_cvar[0:int(ts)],y_best_cvar[0:int(ts)]
                                            , "b", linewidth=3, label="Ego trajectory")    
                    patch_ego = pt.Rectangle((x_best_cvar[int(ts)]-2.5,y_best_cvar[int(ts)]-1.25), 
                                            5,2.5,np.rad2deg(psi_best_cvar[int(ts)]), fc = "orange",ec = "black",linewidth = 2,
                                                label="Ego")
                    axs.add_patch(patch_ego)
                    ...
                    ego_traj, = axs.plot(x_best_saa[0:int(ts)],y_best_saa[0:int(ts)]
                                            , "b", linewidth=3, label="Ego trajectory")    
                    patch_ego = pt.Rectangle((x_best_saa[int(ts)]-2.5,y_best_saa[int(ts)]-1.25), 
                                            5,2.5,np.rad2deg(psi_best_saa[int(ts)]), fc = "orange",ec = "black",linewidth = 2,
                                                label="Ego")
                    axs.add_patch(patch_ego)
                    ...
                    v_best = np.sqrt(xdot_best_saa**2+ydot_best_saa**2)
                ...
                axs.set(xlim=(-3, 105), ylim=(-20, 20))

                axs.text(-2,17,'{}% collisions({}/{})'.format(round(100*_count/_num_batch,2),_count,_num_batch))
                axs.text(-2,10,'Ego speed {} m/s'.format(round(v_best[ts],2)))
                
                patch_obs = pt.Rectangle((_x_obs_noise_shift[0]-2.5,_y_obs_noise_shift[0]-1.25), 5,2.5,
                            fc = "red",lw = lw_ego,label = "Uncertain obstacle positions")
                
                axs.legend(handles=[ego_traj,patch_obs,_traj,lb,patch_ego],loc='upper right',ncol=3,fontsize="medium")

                camera.snap()
        
            animation = camera.animate()
            animation.save('videos/{}_{}_config_{}.mp4'.format(cost,noise,k))
